...
KEYWORDS="-* x86 amd64"
SRC_URI="x86?	( 
			${URI}/${PV}.${MAGIC1}-${MAGIC2}/debian/plexmediaserver_${PV}.${MAGIC1}-${MAGIC2}_i386.deb
		 )
		 amd64?	(
		 	${URI}/${PV}.${MAGIC1}-${MAGIC2}/debian/plexmediaserver_${PV}.${MAGIC1}-${MAGIC2}_amd64.deb
		 )"
...
src_prepare() {
	mkdir data
	tar -xJf data.tar.xz -C data || die "unpack fail"
}

src_install() {
	tar -xJf data.tar.xz -C ${D} || die "main package extract fail"
